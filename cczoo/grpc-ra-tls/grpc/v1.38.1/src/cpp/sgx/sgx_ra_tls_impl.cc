...
std::vector<std::string> generate_key_cert(
    ...
    uint8_t *pubkey_buf = nullptr;
    uint8_t hash[SHA256_DIGEST_LENGTH];
    char privkey_pem[CERT_KEY_MAX_SIZE], cert_pem[CERT_KEY_MAX_SIZE];

    RSA *rsa = RSA_new();
    BIGNUM *e = BN_new();
    BN_set_word(e, RSA_F4);
    RSA_generate_key_ex(rsa, 3072, e, nullptr);

    // private key
    BIO *privkey_bio = BIO_new(BIO_s_mem());
    if (!privkey_bio) {
        grpc_printf("create private key bio failed!");
    }

    ret = PEM_write_bio_RSAPrivateKey(privkey_bio, rsa, nullptr, nullptr, 0, nullptr, nullptr);
    if (ret == 0) {
        grpc_printf("write private key bio failed!");
    }

    ret = BIO_read(privkey_bio, privkey_pem, privkey_bio->num_write);
    if (ret == 0) {
        grpc_printf("read private key bio failed!");
    }

    // public key
    EVP_PKEY *evp_pkey = EVP_PKEY_new();
    EVP_PKEY_assign_RSA(evp_pkey, rsa);
    size_t pkey_size = i2d_PUBKEY(evp_pkey, 0);
    ...
    // size_t pubkey_size = i2d_PUBKEY(pkey, &pubkey_buf);
    size_t pubkey_size = i2d_X509_PUBKEY(X509_get_X509_PUBKEY(x509), &pubkey_buf);
    if (pubkey_size != pkey_size) {
        grpc_printf("verify pubkey size failed!");
    }

    SHA256_CTX sha256;
    SHA256_Init(&sha256);
    SHA256_Update(&sha256, pubkey_buf, pkey_size);
    ...
    X509_sign(x509, evp_pkey, EVP_sha1());

    BIO *pubkey_bio = BIO_new(BIO_s_mem());
    if (!pubkey_bio) {
        grpc_printf("create pubkey bio failed!");
    }

    ret = PEM_write_bio_X509(pubkey_bio, x509);
    if (ret == 0) {
        grpc_printf("write pubkey bio failed!");
    }

    ret = BIO_read(pubkey_bio, cert_pem, pubkey_bio->num_write);
    if (ret == 0) {
        grpc_printf("read pubkey bio failed!");
    }

    std::vector<std::string> key_cert;
    key_cert.emplace_back(std::string((char*) privkey_pem));
...
int verify_pubkey_hash(X509 *x509, uint8_t *pubkey_hash, uint32_t hash_size) {
    ...
    auto key_size = i2d_X509_PUBKEY(X509_get_X509_PUBKEY(x509), &pubkey);

    uint8_t hash[hash_size];
    SHA256_CTX sha256;
    SHA256_Init(&sha256);
    SHA256_Update(&sha256, pubkey, key_size);
    SHA256_Final(hash, &sha256);
