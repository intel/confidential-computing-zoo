...
ARG BASE_IMAGE=tdx-dev:dcap1.19-ubuntu22.04-latest
FROM ${BASE_IMAGE}
...
RUN sed -i "s/std::max(SIGSTKSZ, 65536)/std::max<size_t>(SIGSTKSZ, 65536)/g" ${GRPC_VERSION_PATH}/third_party/abseil-cpp/absl/debugging/failure_signal_handler.cc
