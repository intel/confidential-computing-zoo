...
int cmd_params_process(int argc, char **argv, struct cmd_params *params)
{
	...
	while ((c = getopt_long(argc, argv, _sopts, _lopts, NULL)) != -1) {
		...
		case 'h':
			...
			return -1;
		case 'S':
			if (optarg == NULL || is_str_empty_or_too_long(optarg, MR_STR_LEN)) {
				if ( access(conf, 0) >= 0 ) {
			...
			break;
		case 'd':
			 if (optarg == NULL || is_str_empty_or_too_long(optarg, NUM_STR_LEN)) {
				if ( access(conf, 0) >= 0 ) {
			...
			params->isv_prod_id = atoi(optarg);
			break;
		case 'v':
		 	if (optarg == NULL || is_str_empty_or_too_long(optarg, NUM_STR_LEN)) {
				if ( access(conf, 0) >= 0  ) {
