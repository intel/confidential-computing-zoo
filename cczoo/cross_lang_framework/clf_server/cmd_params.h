...
struct cmd_params {
    char MRSigner[MRSIGNER_LEN];
    char MREnclave[MRENCLAVE_LEN];
    uint16_t isv_prod_id;
    uint16_t isv_svn;
    char secret[WRAP_KEY_SIZE * 2 + 1];
    uint16_t port;
    char server_cert_path[PATH_MAX];
    char server_private_key_path[PATH_MAX];
};
