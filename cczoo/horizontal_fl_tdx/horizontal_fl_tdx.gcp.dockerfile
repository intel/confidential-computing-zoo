...
COPY ${GCP_PATH}/grpc_ratls.gcp.patch ${TF_BUILD_PATH}/third_party/grpc/grpc_ratls.patch

# Apply TensorFlow patch
COPY ${GCP_PATH}/tf_v2.6.diff ${TF_BUILD_PATH}/tf_v2.6.diff
WORKDIR ${TF_BUILD_PATH}
RUN git apply tf_v2.6.diff

# Build and install Google's TDX attestation tools
WORKDIR /
