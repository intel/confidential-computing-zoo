...
class SecretClient {
    public:
        SecretClient(std::shared_ptr<grpc::Channel> channel) : stub_(secretmanager::SecretManger::NewStub(channel)) {}

        std::string GetSecret(const std::string& key) {
            grpc::ClientContext context;
            secretmanager::SecretReply reply;
            secretmanager::SecretRequest request;
    ...
    private:
        std::unique_ptr<secretmanager::SecretManger::Stub> stub_;
};
