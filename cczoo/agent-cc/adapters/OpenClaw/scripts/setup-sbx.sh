...
generate_compose() {
  local host_bind_prefix=""
  if [[ "$OPENCLAW_GATEWAY_BIND" == "loopback" ]]; then
    host_bind_prefix="127.0.0.1:"
  fi
...
services:
  openclaw-gateway:
    ...
    group_add:
      ...
      - "\${DOCKER_GID}"
    ports:
      - "${host_bind_prefix}\${OPENCLAW_GATEWAY_PORT:-18789}:18789"
      - "${host_bind_prefix}\${OPENCLAW_BRIDGE_PORT:-18790}:18790"
    init: true
...
sandbox_ok=true
_sbx_mode="${OPENCLAW_SANDBOX_MODE:-non-main}"
_sbx_scope="${OPENCLAW_SANDBOX_SCOPE:-session}"
_sbx_access="${OPENCLAW_WORKSPACE_ACCESS:-none}"
run_prestart_cli config set agents.defaults.sandbox.mode "$_sbx_mode"        >/dev/null || sandbox_ok=false
run_prestart_cli config set agents.defaults.sandbox.scope "$_sbx_scope"      >/dev/null || sandbox_ok=false
run_prestart_cli config set agents.defaults.sandbox.workspaceAccess "$_sbx_access" >/dev/null || sandbox_ok=false
...
echo "Sandbox config written: mode=$_sbx_mode, scope=$_sbx_scope, workspaceAccess=$_sbx_access"
...
echo " Token          : stored in $ENV_FILE (contents not printed)"
