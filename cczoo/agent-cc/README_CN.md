...
Agent-CC 通过三个相互关联的支柱能力来解决这一问题
- **全生命周期数据保护**（硬件内存加密、基于证明的受控加密存储访问、不可篡改审计日志）
- **从构建到运行时的完整性**（通过 TC-API 将供应链校验与运行时度量关联起来）
- **可信服务组合**（在与外部服务交换敏感数据之前先完成双向证明与信任验证）。
...
<div align="center">图 1：全生命周期数据保护，以 OpenClaw 为例</div>
...
<div align="center">图 2：从构建产物到受证明的运行时</div>
...
核心服务用于实现上述三项架构要求：
- **[TC-API](core/tc_api)**：可信构建、发布与启动控制路径，用于构建到运行时的校验与策略执行。
- **[Trusted Log (TLog)](core/tlog)**：不可篡改、带签名的运行时证据与审计轨迹。
- **Argus（可信服务组合）**：面向跨服务可信接入与可信交互的架构。详细设计将在下一版本补充。

**适配器**

 - **[OpenClaw](./adapters/OpenClaw/README.md)** : 提供在 TDX 可信执行环境中部署 OpenClaw 工作负载的集成路径，通过 LUKS 加密工作区隔离、密码学镜像签名（Cosign/Sigstore）、远程认证（trust-service + KBS）、SBOM 生成与透明度日志、以及端到端构建-运行时信任链控制提供全方位保护——且无需对框架进行侵入性修改。
