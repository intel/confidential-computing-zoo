...
- **硬件**：支持 Intel TDX 的 CPU 和服务器
- **操作系统**：启用 TDX 的 Linux Guest OS（TDVM）
- **依赖服务**：已部署 Trustee（KBS、AS、RVPS）
- **工具链**：Docker/containerd、Cosign、cryptsetup
- **网络**：可访问 Rekor、OCI registry 和 KBS 服务

### 🧩 端到端参考场景（Agent + LLM + Memory）

本指南按具体服务组织。一个典型的 Agent-CC 部署路径如下：

1. **Agent 运行时** 运行在 TDVM 中，例如 OpenClaw。
2. **LLM 服务** 通过具备证明能力且受策略约束的路径访问，例如 Ollama。
3. **记忆服务** 以加密形式持久化，并且只向通过证明的运行时释放。

这样可以形成一条覆盖编排、模型推理与记忆持久化的端到端保护链路。

### 🔌 面向适配器的集成

- **Agent 适配器**：
  - [OpenClaw](adapters/OpenClaw)：用于在共享 Agent-CC 核心服务之上运行 OpenClaw 的部署侧适配入口。
- **LLM 适配器**：
  - Ollama：
...
部署 TDVM 所需基础环境、Trustee 服务以及 TC-API 控制组件。

#### 2. 部署 OpenClaw 机密计算 Agent 适配器

待补充：优先使用 OpenClaw 适配路径，后续其他框架可沿用相同模式。

#### 3. 验证端到端保护流程

验证构建证据、运行时证明和服务访问策略是否实现了端到端执行。

---

**Agent-CC**：将机密计算能力带给运行在 Intel TDX 上的 AI Agents。
