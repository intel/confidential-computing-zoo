...
def criteo_dataprocessing_sample():
    ...
    df_train = df_train[0:int(len(df_train)/2)]
...
def run():
    ...
    if FLAGS.job_name == "ps":
        ...
        with tf.device(tf.train.replica_device_setter(
            ...
            # Val
            # val_out = model.call((val_dense, val_cat))
            ...
            for i in range(EPO):
                ...
                try:
                    ...
                    for j in range(batch_num):
                        ...
                        # print_loss_dict[ret_step] = np.mean(ret_loss)
                ...
                print(f"Loss: {np.mean(loss_list).astype(np.float).round(3)}, acc: {np.mean(train_acc_list).round(3)}, batch time: {round(itr_time/itr_num, 7)} s, batch num: {itr_num}")


        with open("plts/worker0_loss.pkl", "wb") as pkf:
        ...
        plt.savefig('plts/plt_worker0.png')

        with open("plts/worker0_train_acc.pkl", "wb") as pkf1:
