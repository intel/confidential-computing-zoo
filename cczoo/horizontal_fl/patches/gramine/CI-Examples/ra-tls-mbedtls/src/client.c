...
static int my_verify_measurements(const char* mrenclave, const char* mrsigner,
    ...
    bool verify = true;
    if (verify && g_verify_mrenclave && memcmp(mrenclave, g_expected_mrenclave, sizeof(g_expected_mrenclave))) {
      verify = false;
    }

    if (verify && g_verify_mrsigner && memcmp(mrsigner, g_expected_mrsigner, sizeof(g_expected_mrsigner))) {
      verify = false;
    }

    if (verify && g_verify_isv_prod_id && memcmp(isv_prod_id, g_expected_isv_prod_id, sizeof(g_expected_isv_prod_id))) {
      verify = false;
    }

    if (verify && g_verify_isv_svn && memcmp(isv_svn, g_expected_isv_svn, sizeof(g_expected_isv_svn))) {
      verify = false;
    }
