...
class TestDatabaseSchema:
    ...
    def test_get_queue_stats(self, db):
        ...
        insert_record("rec-2", "e2", {}, "FAILED", chain_id="a",
...
class TestSubmitDaemon:
    def test_failed_record_blocks_later_submissions(self, db):
        ...
        insert_record("rec-1", "e1", {"bundle": "{}"}, "FAILED",
    ...
    def test_retry_threshold_transitions_to_failed(self, db):
        ...
        from trusted_container_log.database import increment_retry
        for _ in range(10):
            increment_retry("rec-1", "PENDING", db)
        ...
        # After 10 retries, the daemon would mark it FAILED
        update_status("rec-1", "FAILED", db)
        failed = get_failed_by_chain("a", db)
        assert len(failed) == 1
...
class TestIntegration:
    def test_commit_flow_through_trucon_endpoint(self, db):
        ...
        from trucon import CommitRequest, _sequencer_lock
