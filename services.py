...
class DockerService:
    ...
    def create_lunks_block(self, user_id, tl_signer: ChainedTransparencyLog, passwd, VFS_SIZE, VFS_PATH):
        ...
        cmd = "./create_encrypted_vfs.sh "+VFS_PATH+" "+VFS_SIZE+" "+passwd+" "+MAPPER_DIR+" "+LOOP_DEVICE
        try:
            ...
            if result.wait() == 0:
                ...
                tl_signer.add_entry({"create_encrypted_vfs":"completed"})
                ...
                tl_signer.add_entry({"createLunks":"create_encrypted_vfs failed"})
                return
        except Exception as e:
            logger.debug(f"crate lunks failed.{e}")
            tl_signer.add_entry({"create_lunks_failed":f"e"})
            return

    def mount_lunks_block(self, user_id, tl_signer: ChainedTransparencyLog, MAPPER_DIR, passwd, MOUNT_PATH, VFS_PATH,LOOP_DEVICE):
        cmd = "./mount_encrypted_vfs.sh "+VFS_PATH+" "+MOUNT_PATH+" "+MAPPER_DIR+" "+passwd+" "+LOOP_DEVICE
        try:
            result = subprocess.Popen(cmd, shell=True, stdout=subprocess.PIPE, stderr=subprocess.STDOUT,text=True)
            logger.info(result.stdout.read())
            if result.wait() == 0:
                ...
                tl_signer.add_entry({"mount_encrypted_vfs":"completed"})
                return MOUNT_PATH
            else:
                logger.debug(f"mount_encrypted_vfs failed: {cmd}")
                self.update_lunks_status(user_id, "mounting", step="mount_encrypted_vfs: failed")
                tl_signer.add_entry({"mountLunks":"mount_encrypted_vfs failed"})
                return
        except Exception as e:
            logger.debug(f"mount lunks failed.{e}")
            tl_signer.add_entry({"mount_lunks_failed":f"e"})
            return


    def unmount_lunks_block(self, user_id, tl_signer: ChainedTransparencyLog, MAPPER_DIR, MOUNT_PATH, LOOP_DEVICE):
        MAPPER_PATH=f'/dev/mapper/{MAPPER_DIR}'
        cmd = "./unmount_encrypted_vfs.sh "+" "+MOUNT_PATH+" "+MAPPER_PATH+" "+LOOP_DEVICE
        try:
            result = subprocess.Popen(cmd, shell=True, stdout=subprocess.PIPE, stderr=subprocess.STDOUT,text=True)
            if result.wait() == 0:
                logger.info(result.stdout.read())
                self.update_lunks_status(user_id, "umounting", step="unmount_encrypted_vfs: success")
                tl_signer.add_entry({"unmount_encrypted_vfs":"completed"})
            else:
                ...
                tl_signer.add_entry({"unmountLunks":"unmount_encrypted_vfs failed"})
        except Exception as e:
            logger.debug(f"unmount lunks failed.{e}")
            tl_signer.add_entry({"unmount_lunks_failed":f"e"})
            return
