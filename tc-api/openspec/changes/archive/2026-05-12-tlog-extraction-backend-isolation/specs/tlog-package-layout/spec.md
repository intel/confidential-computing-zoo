...
- **THEN** it SHALL contain the FastAPI sequencer app (`app.py`), SQLite queue operations (`database.py`), and platform-specific adapters under `adapters/` (`tdx_mr.py`, `tdx_quote.py`, `ccel.py`). Immutable-log backend adapters (`sigstore.py`, `oci_mirror.py`) SHALL NOT be present — they live in `tlog-rekor`.
