...
The `GET /status` endpoint SHALL return a JSON object with the following fields: `has_queued_records` (bool), `queued_record_count` (int), `next_record_id` (string or null), `submitting_count` (int), `failed_retryable_count` (int), `failed_terminal_count` (int). The `has_queued_records` field SHALL be `true` when `queued_record_count > 0`. The `next_record_id` field SHALL contain the `record_id` of the pending record with the lowest `sequence_num` that has `rtmr_extended = true`, or `null` if no such record exists.

#### Scenario: Queue with pending and failed records
- **WHEN** the commit queue contains 2 PENDING records, 1 SUBMITTING record, 1 FAILED_RETRYABLE record, and 1 FAILED_TERMINAL record
- **THEN** the response SHALL include `has_queued_records: true`, `queued_record_count: 2`, `submitting_count: 1`, `failed_retryable_count: 1`, `failed_terminal_count: 1`, and `next_record_id` set to the record_id of the lowest-sequence PENDING record

#### Scenario: Empty queue
- **WHEN** the commit queue contains no records
- **THEN** the response SHALL include `has_queued_records: false`, `queued_record_count: 0`, `next_record_id: null`, `submitting_count: 0`, `failed_retryable_count: 0`, `failed_terminal_count: 0`
