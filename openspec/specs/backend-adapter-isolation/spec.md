...
The TruCon submit daemon SHALL select and instantiate the appropriate `ImmutableLogAdapter` implementation based on configuration, using direct conditional imports rather than a plugin registry.

#### Scenario: Default backend is rekor
- **WHEN** TruCon starts without explicit backend configuration
- **THEN** the submit daemon SHALL load `SigstoreLogAdapter` from `tlog_rekor.adapter`

#### Scenario: Backend selection via environment variable
- **WHEN** `TC_IMMUTABLE_BACKEND` is set to a supported value (e.g., `rekor`, `onchain`)
...
- **WHEN** `TC_IMMUTABLE_BACKEND` is set to an unsupported value
