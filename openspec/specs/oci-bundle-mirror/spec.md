...
#### Scenario: Attestation storage success does not require mirror publication for verification
- **WHEN** public Rekor replay can already materialize the required historical payload facts through attestation storage
- **THEN** verification SHALL NOT require OCI mirror participation unless an explicit mirror-required policy is configured
